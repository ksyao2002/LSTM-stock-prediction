...
class DeepStockTraderEnv(gym.Env):
  ...
  def total(self, timestep=-1, open=True):
    return self.cash + self.n_shares * self.data[timestep, self.columns_map["open_today" if open else "close_prev"]]

  def step(self, action):

    # ********************** EXECUTE ACTION **********************
    open_j = self.columns_map["open_today"]
    close_j = self.columns_map["close_prev"]
    if action == self.BUY:
        # Execute transaction at today's opening price
    ...
    # IMPORTANT 
    # We define reward to be (total account value at close) - (total account value at open_today)
    # Basically your reward is the amount gained over the course of the day 
    reward = self.total(self.timestep+1, open=False) - self.total(self.timestep)
